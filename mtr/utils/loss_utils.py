...
def gen_smooth_heatmap_target(
    width: int, height: int, indices_gt: torch.Tensor, gaussian_std: float = 4.0
) -> torch.Tensor:
    """Generates smooth heatmap target using the Gaussian kernel.

    Args:
        width (int): the width of heatmap output.
        height (int): the height of heatmap output.
        indices_gt (torch.Tensor): the ground-truth indices of the shape (n_heatmaps, 2)
        gaussian_std (float, optional): the standard deviation of the Gaussian kernel. Defaults to 4.0.

    Returns:
        torch.Tensor: smooth heatmap target
    """
    assert indices_gt.shape[1] == 2
    indices_w = torch.arange(width).float()
    indices_h = torch.arange(height).float()
    coord = torch.cartesian_prod(indices_w, indices_h)  # (W*H, 2)

    # Target is of (B, W, H)
    delta_x = coord[None, :, 0] - indices_gt[:, None, 0]  # (B, W*H)
    delta_y = coord[None, :, 1] - indices_gt[:, None, 1]  # (B, W*H)
    target = torch.exp(-(delta_x**2 + delta_y**2) / (2 * gaussian_std**2))
    return target.reshape((indices_gt.shape[0], width, height))
...
def pixelwise_focal_loss(
    ...
    loss_all = -(1 - target)**beta * prob**alpha * torch.log(1 - prob + 1e-6)
    # Updates the loss of the ground-truth pixels.
    loss_all[indices_p, indices_gt[:, 0], indices_gt[:, 1]] = -(1 - prob_gt)**alpha * torch.log(prob_gt + 1e-6)
