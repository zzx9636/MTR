...
# Adapted from waymax/metrics/comfort.py
...
class KinematicsFeasibilityMetric(abstract_metric.AbstractMetric):
  """Kinematics Feasibility metric.
  ...
  transition. This metric returns a POSITIVE VALUE if the magnitude of the estimated
  ...
  def compute(
    ...
    Returns:
      A (..., num_objects) MetricResult containing the metric result, with
        values indicates that the current step violates the kinematic 
        infeasibility metrics, and a negative value indicates that the
