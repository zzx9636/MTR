...
class OverlapMetric(abstract_metric.AbstractMetric):
    ...
    def compute_overlap(
        ...
        # Shape: (num_objects, 1, 5)
        
        # Get corners of the bounding boxes
        # Shape: (num_objects, 4, 2)
        corners = geometry.corners_from_bboxes(traj_5dof[..., 0, :])
        corners_all = corners[..., None, :, :].repeat(num_obj, axis=1) # Shape: (num_objects, num_objects, 4, 2)
        ...
        np.fill_diagonal(signed_distance, 1e3)
