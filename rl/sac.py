...
from rl.base_training import BaseTraining
...
class SAC(BaseTraining):
  ...
  def interact(self, 
    ...
    # TODO: 
    # 1. Run a forward pass of the encoder to get the latent encoding.
    # 2. Run a forward pass of the decoder to get action
    # 3. Step the environment with the action and get reward
    # 4. Check if the environment is done
    # 5. Store the transition in memory
    # 6. Return the state for the next step
  ...
  def update_one(self, batch: Batch, timer: int) -> Tuple[float, float, float, float]:
    ...
    if timer % self.actor.update_period == 0:  # Updates the actor.
      ...
      loss_pi, loss_ent, loss_alpha = self.actor.update(q1=q1_sample, q2=q2_sample, log_prob=log_prob)
    else:
